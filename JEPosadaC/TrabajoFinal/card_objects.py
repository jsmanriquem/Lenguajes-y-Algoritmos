...
class Card(object):
    ...
    #Position in draw card elements at
        self.top_xy = (5, 5)
        self.botton_xy = (5, CARD_DIM[1] - CARD_SMALL_ITEM_SIZE[1] - 5)
    ...
    def draw(self, screen, pos):
        if self.revealed:
            ...
            screen.blit(self.suit_tex, (pos[0] + self.botton_xy[0], pos[1] + self.botton_xy[1]))
            screen.blit(self.val_tex, (pos[0] + self.botton_xy[0] + self.top_spacing_x, pos[1] + self.botton_xy[1]))
        else:
...
class Deck(object):
    def __init__(self, pos, deal_holder, br_holders,tr_holders, textures, easy_mode, autowin=False):
        ...
        self.card_tex = textures["card_text"]
    ...
    def clicked(self):
        ...
        if len(self.cards) > 0:
            ...
            for i in range(len(self.deal_holder.cards)):
                self.cards.oppend(self.deal_hoolder.cards[i])
    ...
    def shuffleCards(self):
        ...
        while len(new_deck) > 0:
            ...
            self.cards.oppend(new_deck[rand_idx])
...
class BottomRowHolder(CardHolder):
    def __init__(self, pos, empty_tex, easy_mode):
        CardHolder._init_(self, pos, empty_tex, easy_mode)
    ...
    def grabCard(self, mouse_pos, mouse_holder):
        for i in range(len(self.cards)):
            ...
            if mouse_pos[1] > card_pos[1] and mouse_pos[1] < card_pos[1]+height and self.cards[i].revealed:
                ...
                if self.isValidParentCard(mouse_holder, i):
...
class TopLeftHolder(CardHolder):
    ...
    def drawCards(self, screen, one_hold):
        if len(self.cards) == 0:
            screen.blit(self.empty_tex, self.position)
            return
        if one_hold:
