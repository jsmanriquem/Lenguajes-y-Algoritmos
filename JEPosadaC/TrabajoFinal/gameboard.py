...
class GameBoard(object):
    def __init__(self, textures, easy_mode, autowin):
        ...
        self.br_card_holders = []
        self.trr_card_holders = []
        self.tl_card_holder = None
    ...
    def mouseClicked(self):
        ...
        if self.mouse_pos[1] > BOTTOMROW_XY[1]:
            ...
            if self.deck.inBounds(self.mouse_pos):
                ...
                tl_holderwidth = CARD_DIM[0] + min(2, len(self.tl_card_holder.cards))*TOPLEFTHOLDER_OFFSET[0]
                if self.mouse_pos[0] > self.tl_card_holder.position[0] and self.mouse_pos[0] < self.tl_card_holder.position[0] + tl_holderwidth\
    ...
    def mouseReleased(self):
        ...
        card_contact_point = (self.mouse_pos[0] + self.mouse_holder_relative_position[0] + int(CARD_DIM[0/2]), \
                              self.mouse_pos[1] + self.mouse_holder_relative_position[1] + int(CARD_DIM[1]/2))
        new_holder = check_win = False
#Bottom row
        if self.mouse_pos[1] > BOTTOMROW_XY[1]:
            for i in range(len(self.br_holder_hor_ranges)):
                if card_contact_point[0] > self.tr_holder_hor_ranges[i][0] and card_contact_point[0] < self.br_holder_hor_ranges[i][1]:
